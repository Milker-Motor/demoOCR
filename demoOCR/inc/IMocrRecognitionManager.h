...
// Sets the number of threads using in process of recognition. Must be greater than zero.
- (void) setRecognizerThreadsCount:(int) threadsCount;
...
// Recognize image region as text block.
- (BOOL) recognizeTextRegion:(UIImage*)image
				  withRegion:(CMocrImageRegion*)region
			  withAttributes:(TMocrTextBlockAttributes) blockAttributes
					callback:(NSObject<IMocrRecognitionCallback>*)callbackObject
				 storeLayout:(CMocrLayout**)layout;
...
// Recognize all barcodes in image.
- (BOOL) extractBarcodesOnImage:(UIImage*)image
					withCallback:(NSObject<IMocrRecognitionCallback>*)callbackObject
					storeLayout:(CMocrLayout**)layout;
